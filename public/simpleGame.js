...
function Sprite(scene, imageFile, width, height){ 
  ...
  this.label.innerHTML = 'Me';
  this.label.position = "absolute";
  ...
  */
  ...
  this.willCollide = function(sprite,dx,dy){
    //check for collision with another sprite    
    //collisions only activated when both sprites are visible
    collision = false;
    if (this.visible){      
      if (sprite.visible){
	        //define borders
        	myLeft      = this.x   - (this.width    / 2) + dx;
	        myRight     = this.x   + (this.width    / 2) + dx;
	        myTop       = this.y   - (this.height   / 2) + dy;
	        myBottom    = this.y   + (this.height   / 2) + dy;
        	otherLeft   = sprite.x - (sprite.width  / 2);
	        otherRight  = sprite.x + (sprite.width  / 2);
	        otherTop    = sprite.y - (sprite.height / 2);
	        otherBottom = sprite.y + (sprite.height / 2);
  ...
  this.onScreen = function(dx,dy){
    ...
    if (this.visible){      
      ...
      if (myLeft < 0) {
        stillOn = false;
      } else if (myRight > 1000) {
        stillOn = false;
      } else if (myTop < 0) {
        stillOn = false;
      } else if (myBottom > 800) { 
        stillOn = false;
      }
    } 
    return stillOn;
...
K_6 = 54; K_7 = 55; K_8 = 56; K_9 = 57; K_CR = 13;
